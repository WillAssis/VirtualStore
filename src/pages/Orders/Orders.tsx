...
function Orders() {
  ...
  const { orders, loading, error, deleteOrder } = useOrders('user');

  const popupUserForDeletion = (id: string) => {
    setSelectedId(id);
      ...
      <Loading loading={loading} error={error}>
        <Container>
          <div className={styles.contentWrapper}>
            <Title icon={icon} text="Meus pedidos" />
            <div className={styles.ordersWrapper}>
              {orders.length > 0 ? (
                <>
                  <Table orders={orders} deleteOrder={popupUserForDeletion} />
                  <Popup
                    ref={popupRef}
                    onAccept={deleteSelectedOrder}
                    onRefuse={() => {}}
                  >
                ...
                <div className={styles.noOrders}>
                  ...
                  <Button path="/produtos">Explorar produtos</Button>
                </div>
              )}
            </div>
          </div>
