        ...
        <ul className="image-selector">
          {product.images.map((_img, index) => (
            <li key={product.id + index}>
              <button
                className={currentImageIndex === index ? 'product-image-selector active' : 'product-image-selector'}
                aria-label={`Selecionar imagem ${index + 1} de ${product.images.length}`}
      ...
      <div className="product-info">
        ...
        <div className="product-controls">
          <Link to={`/admin/produtos/editar/${product.slug}`}>
            <svg aria-label="Editar produto" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
              <title>Editar</title>
          ...
          <button onClick={() => deleteProduct(product.slug)} aria-label="Deletar produto">
